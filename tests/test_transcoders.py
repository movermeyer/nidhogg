...
class TestDatetimeEncoderDecoder(unittest.TestCase):
    ...
    def test_time(self):
        ...
        self.assertEqual(obj,
                         json.loads(string, object_hook=json_datetime_hook))
        self.assertEqual(obj_tz,
                         json.loads(string_tz, object_hook=json_datetime_hook))
        self.assertIsNotNone(
            json.loads(string_tz, object_hook=json_datetime_hook).tzinfo)

    def test_complex(self):
        obj = {'id': '1', 'params': (datetime(2014, 6, 17, 9, 38, 39, 911853),),
               'jsonrpc': 2.0, 'method': 'w'}
